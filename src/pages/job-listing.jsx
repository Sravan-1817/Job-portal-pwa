...
import { getJobs, getRecommendedJobs, getRecentJobs, getMyJobs } from "@/api/apiJobs";
...
const JobListing = () => {
  ...
  const { loading: loadingJobs, data: jobs, fn: fnJobs } = useFetch(
    user?.unsafeMetadata?.role === "recruiter" ? getMyJobs : getJobs,
    {
      location: "",
      company_id: "",
      searchQuery: "",
  ...
  }, [jobs, appliedJobs, user]);

  // Filter out applied jobs from recent jobs
  const filteredRecentJobs = useMemo(() => {
    return recentJobs.filter(job => !appliedJobs.includes(job.id));
  }, [recentJobs, appliedJobs]);

  // Filter out applied jobs from recommended jobs
  const filteredRecommendedJobs = useMemo(() => {
    return recommendedJobs.filter(job => !appliedJobs.includes(job.id));
  }, [recommendedJobs, appliedJobs]);

  // Calculate pagination for all jobs
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      // Always fetch jobs, regardless of applied jobs
      fnJobs();
    } catch (error) {
  ...
  const handleSearch = (e) => {
    ...
    const searchValue = formData.get("search-query") || "";
  ...
  // Effect to refetch jobs when filters change
  useEffect(() => {
    if (isLoaded && user?.id) {
      const filterOptions = {
        location: location,
      ...
      console.log("Applying filters:", filterOptions); // Debug log
      fnJobs(filterOptions);
    }
  }, [searchQuery, location, company_id, appliedJobs, isLoaded, user?.id]);
